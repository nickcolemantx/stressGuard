...
interface AlertEventDao {
    ...
    @Query("SELECT * FROM alert_events WHERE timestampMs BETWEEN :fromMs AND :toMs ORDER BY timestampMs DESC")
    suspend fun between(fromMs: Long, toMs: Long): List<AlertEventEntity>

    @Query("SELECT * FROM alert_events ORDER BY timestampMs DESC LIMIT :n")
    fun recent(n: Int): Flow<List<AlertEventEntity>>
